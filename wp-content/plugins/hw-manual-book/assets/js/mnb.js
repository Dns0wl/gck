...
if (app) {
  ...
  async function handleProcessSubmit(event) {
    ...
    try {
      submitProcessForm(windowName, customerName, finalDate, Boolean(previewWindow));
      closeModal();
    } catch (error) {
      console.error(error);
  ...
  function submitProcessForm(targetWindowName, customerName, orderDate, hasPreviewWindow) {
    if (!HWMBApp.processUrl || !HWMBApp.processNonce || !state.currentItem) {
      throw new Error('Process endpoint not available');
    }
    const form = document.createElement('form');
    form.method = 'POST';
    form.action = HWMBApp.processUrl;
    form.target = hasPreviewWindow ? targetWindowName : '_blank';
    form.style.display = 'none';
    addHiddenInput(form, '_wpnonce', HWMBApp.processNonce);
    addHiddenInput(form, 'post_id', state.currentItem.id);
    addHiddenInput(form, 'customer_name', customerName);
    addHiddenInput(form, 'order_date', orderDate);
